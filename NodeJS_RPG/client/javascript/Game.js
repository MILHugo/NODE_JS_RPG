...
function create() {
    if (document.getElementById('name').value != "" && document.getElementById('hp').value != "" &&
        document.getElementById('strength').value != "" && document.getElementById('agility').value != "" &&
        ...
        alert('FAIS PAS LE CON PHILIPPE! JE SAIS OU TU TE CACHES!' +
            '\nRemplis les cases comme tout le monde...');
    }
...
function load() {
    ...
    req.onreadystatechange = function () {
        if (req.readyState === 4 && req.status === 200) {
            ...
            if (joueur.hp <= 0) document.getElementById('styleBtn').innerHTML = "#direction button, #load button{cursor: not-allowed;pointer-events: none;opacity: 0.5;}";
            if (joueur.hp > 0) document.getElementById('styleBtn').innerHTML = "#direction button, #load button{cursor: allowed;pointer-events: all;opacity: 1;}";
            actualiseALL();
        }
    };
...
function finDePartie() {
    document.getElementById('body').innerHTML = "<div id='test' style='font-size : 16em;'><b>taperdu</b></div><button onclick ='location.href = \"../html_css/Game.html\"'>Retry</button><style>#test{user-select: none;}</style>";
